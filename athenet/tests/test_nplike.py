...
from numpy.testing import assert_array_equal as are, \
    assert_array_almost_equal as array_almost_equal
...
class NumlikeTest(unittest.TestCase):
    ...
    def test_neg(self):
        a, n = self.prepare((2, 3))
        array_almost_equal(-a, n.neg().eval())
    ...
    def test_square(self):
        a, n = self.prepare((2, 3))
        array_almost_equal(a * a, n.square().eval())

    def test_power(self):
        a, n = self.prepare((2, 3))
        e = 3.1415
        array_almost_equal(np.power(a, e), n.power(e).eval())

    def test_dot(self):
        a, n = self.prepare((2, 3))
        b, m = self.prepare((3, 4))
        p = n.dot(b).eval()
        array_almost_equal(p, A([[20, 23, 26, 29], [56, 68, 80, 92]]))

    def test_max(self):
        a, n = self.prepare((2, 3))
        b = A([[3, 0, 3], [0, 3, -123]])
        array_almost_equal(n.max(b).eval(), A([[3, 1, 3], [3, 4, 5]]))
    ...
    def test_amax(self):
        ...
        b1 = m.eval()
        b2 = m2.eval()
        array_almost_equal(b1, A([[3, 4, 5]]))
        array_almost_equal(b2, A([[2], [5]]))

    def test_reshape(self):
        a, n = self.prepare((2, 3))
        m = n.reshape((6, 1))
        array_almost_equal(m.eval(), A([[0], [1], [2], [3], [4], [5]]))
    ...
    def test_sum(self):
        a, n = self.prepare((2, 3))
        s = n.sum()
        array_almost_equal(s.eval(), A([15]))

    def test_abs(self):
        a, n = self.prepare((2, 3))
        n -= 3
        s = n.abs()
        array_almost_equal(s.eval(), A([[3, 2, 1], [0, 1, 2]]))

    def test_T(self):
        a, n = self.prepare((2, 3))
        m = n.T
        array_almost_equal(m.eval(), A([[0, 3], [1, 4], [2, 5]]))
